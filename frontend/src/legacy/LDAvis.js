...
// === Modernization: Hotjar user-study tracker removed (added during Vite/TS migration) ===
// The original paper deployment loaded the Hotjar snippet in <head> and
// called `window.hj('identify', ...)` here to record user-study sessions.
// The open-source library does not (and must not) ship analytics, so the
// call is gone. `actions_across_time` is still populated above for callers
// that want to introspect interaction history programmatically.
