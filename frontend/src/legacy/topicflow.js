d3.topicflow = function() {
    ...
    topicflow.link = function() {
      ...
      function link(d) {
        // === Modernization: comma-vs-semicolon typo fix (added during Vite/TS migration) ===
        // The original had `y0 = ... ;` (semicolon) followed by `y1 = ...`
        // which made y1 an implicit window global. Strict-mode bundler
        // scope rejects this with ReferenceError, so we replace `;` with
        // `,` to keep both inside the same `var` declaration. Behaviour is
        // identical because nothing else reads y1 before this line.
